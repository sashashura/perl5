/*    util.h
 *
 *    Copyright (C) 1991, 1992, 1993, 1999, 2001, 2002, 2003, 2004, 2005, 2007,
...
/* outside the core, perl.h undefs HAS_QUAD if IV isn't 64-bit We can't swap
   this to HAS_QUAD, because the logic here affects the type of perl_drand48_t
   below, and that is visible outside of the core. */
#if defined(U64TYPE)
...
/* U64 is only defined under PERL_CORE, but this needs to be visible elsewhere
 * so the definition of PerlInterpreter is complete.
 */
...
typedef struct {
    ...
    UV frame_count;
    /* The total size of the Perl_c_backtrace, including this header, the
     * frames, and the name strings. */
    UV total_bytes;
...
typedef struct {
    ...
    /* The offsets are from the beginning of the whole backtrace, which makes
     * the backtrace relocatable. */
     ...
     * Objective-C even if the API would be known). Currently we open read pipe
     * to "xcrun atos" and parse the output - quite disgusting. And that won't
     * work if the Developer Tools isn't installed. */

    /* FreeBSD notes: execinfo.h exists, but probably would need also the
     * library -lexecinfo. BFD exists if the pkg devel/binutils has been
     * installed, but there seems to be a known problem that the "bfd.h"
     * getting installed refers to "ansidecl.h", which doesn't get installed.
     */

    /* Win32 notes: as moral equivalents of backtrace() + dladdr(), one could
     * possibly first use GetCurrentProcess() + SymInitialize(), and then
     * CaptureStackBackTrace() + SymFromAddr(). */

    /* Note that using the compiler optimizer easily leads into much of this
     * information, like the symbol names (think inlining), and source code
     * locations getting lost or confused. In many cases keeping the debug
     * information (-g) is necessary.
     *
    ...
    /* For example: the mere presence of <bfd.h> is no guarantee: e.g. OS X has
     * that, but BFD does not seem to work on the OSX executables.
     *
...
typedef struct {
    ...
    /* After the header come: (1) header.frame_count frames (2) frame_count
     * times the \0-terminated strings (object_name and so forth). The frames
     * contain the pointers to the starts of these strings, and the lengths of
     * these strings. */
} Perl_c_backtrace;
 ...
 */
#define HSm_APIVERLEN   0x0000001F  /* perl version string won't be more than
                                    31 chars */
#define HS_APIVERLEN_MAX    HSm_APIVERLEN
#define HSm_XSVERLEN        0x0000FF00  /* if 0, not present, dont check, die
                                           if over 255 */
...
#define HSf_POPMARK         0x00000040  /* popmark mode or you must supply ax
                                           and items */
#define HSf_IMP_CXT         0x00000080  /* ABI, threaded, MULTIPLICITY, pTHX_
                                           present */
#define HSm_INTRPSIZE       0xFFFF0000  /* ABI, interp struct size */
/* A mask of bits in the key which must always match between a XS mod and
   interp. Also if all ABI bits in a key are true, skip all ABI checks, it is
   very the unlikely interp size will all 1 bits */
/* Maybe HSm_APIVERLEN one day if Perl_xs_apiversion_bootcheck is changed to a
   memcmp */
#define HSm_KEY_MATCH                                                       (HSm_INTRPSIZE|HSf_IMP_CXT)
#define HSf_NOCHK                                                           HSm_KEY_MATCH               \
    /* if all ABI bits are 1 in the key, dont chk */


#define HS_GETINTERPSIZE(key)                                               ((key) >> 16)
/* if in the future "" and NULL must be separated, XSVERLEN would be 0 means
arg not present, 1 is empty string/null byte */
...
/* internal to util.h macro to create a packed handshake key, all args must be
   constants */
/* U32 return = (U16 interpsize, bool cxt, bool setxsubfn, bool popmark, U5
   (FIVE!) apiverlen, U8 xsverlen) */
