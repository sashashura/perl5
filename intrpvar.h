...
/* These variables are per-interpreter in threaded/multiplicity builds, global
 * otherwise.
 *
 ...
 * generate the appropriate export list for win32. If the variable needs to be
 * initialized, use PERLVARI.
 *
...
PERLVAR(I, markstack,   I32 *)          /* stack_sp locations we're remembering
                                         */
...
PERLVARI(I, hash_rand_bits_enabled, U8, 1) /* used to randomize hash stuff 0.
                                              no-random 1. random 2.
                                              determinsitic */
#endif
PERLVARI(I, hash_rand_bits, UV, 0)      /* used to randomize hash stuff */
#endif
PERLVAR(I, strtab,      HV *)           /* shared string table */
/* prog counter for the currently executing OP_MULTIDEREF Used to signal to
 * S_find_uninit_var() where we are */
...
However, C<$^W> is treated as a boolean, whereas C<PL_dowarn> is a collection
of flag bits.
...
/* the currently active slab in a chain of slabs of regmatch states, and the
 * currently active state within that slab. This stack of states is shared
 * amongst re-entrant calls to the regex engine */
...
This is the C<false> SV. It is readonly. See C<L</PL_sv_yes>>. Always refer to
this as C<&PL_sv_no>.
...
similar to C<L</PL_sv_no>> except for its string value. Can be used as a cheap
alternative to C<mXPUSHi(0)> for example. Always refer to this as
...
#else
/* store the immortals as an array to ensure they are contiguous in memory:
 * makes SvIMMORTAL_INTERP(sv) possible */
...
PERLVAR(I, na,          STRLEN)         /* for use in SvPV when length is Not
                                           Applicable */
...
Call C<abort()> on exit. This is used internally by perl itself to abort if
exit is called while processing exit.
...
small-scale optimisations. The function is called once for each subroutine that
is compiled, and is passed, as sole parameter, a pointer to the op that is the
entry point to the subroutine. It modifies the op tree in place.
...
L<perlguts/Compile pass 3: peephole optimization>. If the new code wishes to
operate on ops throughout the subroutine's structure, rather than just at the
top level, it is likely to be more convenient to wrap the L</PL_rpeepp> hook.
...
side chain. It is passed, as sole parameter, a pointer to the op that is at the
head of the chain. It modifies the op tree in place.
...
extensions to free any extra attribute they have locally attached to an OP. It
is also assured to first fire for the parent OP and then for its kids.
...
SV which holds the name of the sub being debugged. This is the C variable which
corresponds to Perl's $DB::sub variable. See C<L</PL_DBsingle>>.
...
PERLVAR(I, dbargs,      AV *)           /* args to call listed by caller
                                           function */
...
PERLVAR(I, globalstash, HV *)           /* global keyword overrides imported
                                           here */
...
PERLVARI(I, gensym,     I32,    0)      /* next symbol for getsym() to define
                                        */
PERLVARI(I, cv_has_eval, bool, FALSE)   /* PL_compcv includes an entereval or
                                           similar */
...
extensions that need to keep information on a per-interpreter basis. In a
pinch, it can also be used as a symbol table for extensions to share data among
each other. It is a good idea to use keys prefixed by the package name of the
...
PERLVAR(I, delaymagic_uid,      Uid_t)  /* current real user id, only for
                                           delaymagic */
PERLVAR(I, delaymagic_euid,     Uid_t)  /* current effective user id, only for
                                           delaymagic */
PERLVAR(I, delaymagic_gid,      Gid_t)  /* current real group id, only for
                                           delaymagic */
PERLVAR(I, delaymagic_egid,     Gid_t)  /* current effective group id, only for
                                           delaymagic */
...
/* these two are provided only to solve library linkage issues; they should not
 * be hooked by user code */
...
PERLVAR(I, padix,       PADOFFSET)      /* lowest unused index - 1 in current
                                           "register" pad */
...
/* current phase the interpreter is in for ordering this structure to remove
   holes, we're assuming that this is 4 bytes. */
...
If C<$ENV{PERL_DESTRUCT_LEVEL}> is set to an integer greater than the value of
C<PL_perl_destruct_level> its value is used instead.
...
/* Array of names of signals, indexed by signal number, for (re)use as the
   first argument to a signal handler. Only one block of memory is allocated
   for both psig_name and psig_ptr. */
...
PERLVAR(I, regex_padav,   AV *)         /* All regex objects, indexed via the
                                           values in op_pmoffset of pmop. Entry
                                           0 is an SV whose PV is a "packed"
                                           list of IVs listing the now-free
                                           slots in the array */
...
PERLVAR(I, debug_pad,   struct perl_debug_pad)  /* always needed because of the
                                                   re extension */
 ...
 * returned by rand() isn't modified by perl's internal use of randomness. This
 * is important if the user has called srand() with a seed.
 */
...
/* The most recently seen `use VERSION` declaration, encoded in a single U16 as
 * (major << 8) | minor. We do this rather than store an entire SV version
 * object so we can fit the U16 into the uv of a SAVEHINTS and not have to
 * worry about SV refcounts during scope enter/exit. */
