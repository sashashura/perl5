...
ALIAS:
    ...
    sv_magic_baz = 2
CODE:
    sv_magicext(sv, NULL, ix == 2 ? PERL_MAGIC_value : PERL_MAGIC_ext, ix ? &vtbl_bar : &vtbl_foo, (const char *)thingy, 0);
...
CODE:
	...
	if (SvTYPE(sv) >= SVt_PVMG) {
		MAGIC *mg = mg_findext(sv, ix == 2 ? PERL_MAGIC_value : PERL_MAGIC_ext, ix ? &vtbl_bar : &vtbl_foo);
		if (mg)
...
ALIAS:
    ...
    sv_unmagic_baz = 2
CODE:
    sv_unmagicext(sv, ix == 2 ? PERL_MAGIC_value : PERL_MAGIC_ext, ix ? &vtbl_bar : &vtbl_foo);
