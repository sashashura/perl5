...
/*
  Set in Perl_pmruntime if op_flags & OPf_SPECIAL, i.e. split. Will be used by
  regex engines to check whether they should set RXf_SKIPWHITE
*/
...
#define RXf_PMf_COMPILETIME \
    (RXf_PMf_MULTILINE|RXf_PMf_SINGLELINE|RXf_PMf_FOLD|RXf_PMf_EXTENDED|RXf_PMf_EXTENDED_MORE|RXf_PMf_KEEPCOPY|RXf_PMf_NOCAPTURE|RXf_PMf_CHARSET|RXf_PMf_STRICT)
 ...
 *  directly in the #define because doing so confuses regcomp.pl. (2**n - 1) is
 *  n 1 bits, so the below gets the contiguous bits between the beginning and
 *  ending shifts */
