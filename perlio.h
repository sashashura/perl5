...
#define PERLIO_H_
/*
Interface for perl to IO functions. There is a hierarchy of Configure
determined #define controls: USE_STDIO - No longer available via Configure.
Formerly forced PerlIO_xxx() to be #define-d onto stdio functions. Now
...
/*
 * Although we may not want stdio to be used including <stdio.h> here avoids
 * issues where stdio.h has strange side effects
*/
#include <stdio.h>
...
/* We now need to determine what happens if source trys to use stdio. There are
 * three cases based on PERLIO_NOT_STDIO which XS code can set how it wants.
 */

#ifdef PERL_CORE
/* Make a choice for perl core code - currently this is set to try and catch
   lingering raw stdio calls. This is a known issue with some non UNIX ports
   which still use "native" stdio features.
 */
...
#if PERLIO_NOT_STDIO
/*
 * PERLIO_NOT_STDIO #define'd as 1 Case 1: Strong denial of stdio - make all
 * stdio calls (we can think of) errors
*/
#include "nostdio.h"
#else                           /* if PERLIO_NOT_STDIO */
/*
 * PERLIO_NOT_STDIO #define'd as 0 Case 2: Declares that both PerlIO and stdio
 * can be used
*/
#endif                          /* if PERLIO_NOT_STDIO */
#else                           /* ifdef PERLIO_NOT_STDIO */
/*
 * PERLIO_NOT_STDIO not defined Case 3: Try and fake stdio calls as PerlIO
 * calls
*/
#include "fakesdio.h"
