...
C<SP> macro. See C<L</SP>>.

=for apidoc m;||djSP

Declare Just C<SP>. This is actually identical to C<dSP>, and declares a local
copy of perl's stack pointer, available via the C<SP> macro. See
C<L<perlapi/SP>>. (Available for backward source code compatibility with the
...
C<TARG> is short for "target". It is an entry in the pad that an OPs C<op_targ>
refers to. It is scratchpad space, often used as a return value for the OP, but
some use it for other purposes.
...
Closing bracket for XSUB arguments. This is usually handled by C<xsubpp>. See
C<L</PUSHMARK>> and L<perlcall> for other uses.
...
handle 'set' magic. Does not use C<TARG>. See also C<L</PUSHmortal>>,
...
Handles 'set' magic. Uses C<TARG>, so C<dTARGET> or C<dXSTARG> should be called
to declare it. Do not call multiple C<TARG>-oriented macros to return lists
from XSUB's - see C<L</mPUSHn>> instead. See also C<L</XPUSHn>> and
C<L</mXPUSHn>>.
...
Handles 'set' magic. Uses C<TARG>, so C<dTARGET> or C<dXSTARG> should be called
to declare it. Do not call multiple C<TARG>-oriented macros to return lists
from XSUB's - see C<L</mPUSHi>> instead. See also C<L</XPUSHi>> and
C<L</mXPUSHi>>.

=for apidoc Am|void|PUSHu|UV uv
Push an unsigned integer onto the stack. The stack must have room for this
element. Handles 'set' magic. Uses C<TARG>, so C<dTARGET> or C<dXSTARG> should
be called to declare it. Do not call multiple C<TARG>-oriented macros to return
lists from XSUB's - see C<L</mPUSHu>> instead. See also C<L</XPUSHu>> and
C<L</mXPUSHu>>.
...
magic. Uses C<TARG>, so C<dTARGET> or C<dXSTARG> should be called to declare
it. Do not call multiple C<TARG>-oriented macros to return lists from XSUB's -
see C<L</mXPUSHi>> instead. See also C<L</PUSHi>> and C<L</mPUSHi>>.

=for apidoc Am|void|XPUSHu|UV uv
Push an unsigned integer onto the stack, extending the stack if necessary.
Handles 'set' magic. Uses C<TARG>, so C<dTARGET> or C<dXSTARG> should be called
to declare it. Do not call multiple C<TARG>-oriented macros to return lists
from XSUB's - see C<L</mXPUSHu>> instead. See also C<L</PUSHu>> and
C<L</mPUSHu>>.

=for apidoc Am|void|mPUSHs|SV* sv
Push an SV onto the stack and mortalizes the SV. The stack must have room for
this element. Does not use C<TARG>. See also C<L</PUSHs>> and C<L</mXPUSHs>>.

=for apidoc Amn|void|PUSHmortal
Push a new mortal SV onto the stack. The stack must have room for this element.
Does not use C<TARG>. See also C<L</PUSHs>>, C<L</XPUSHmortal>> and
C<L</XPUSHs>>.
...
Push an integer onto the stack. The stack must have room for this element. Does
not use C<TARG>. See also C<L</PUSHi>>, C<L</mXPUSHi>> and C<L</XPUSHi>>.
...
use C<TARG>. See also C<L</XPUSHs>>, C<L</PUSHmortal>> and C<L</PUSHs>>.
...
Push an unsigned integer onto the stack, extending the stack if necessary. Does
not use C<TARG>. See also C<L</XPUSHu>>, C<L</mPUSHu>> and C<L</PUSHu>>.
...
/* _EXTEND_SAFE_N(n): private helper macro for EXTEND(). Tests whether the
 * value of n would be truncated when implicitly cast to SSize_t as an arg to
 * stack_grow(). If so, sets it to -1 instead to trigger a panic. It will be
 * constant folded on platforms where this can't happen.
 */
 ...
 * stack_grow() to be skipped. On the other hand, false positives are safe.
 * Bear in mind that sizeof(p) may be less than, equal to, or greater than
 * sizeof(n), and while n is documented to be signed, someone might pass an
 * unsigned value or expression. In general don't use casts to avoid warnings;
 * instead expect the caller to fix their code. It is legal for p to be greater
 * than PL_stack_max. If the allocated stack is already very large but current
 * usage is small, then PL_stack_max - p might wrap round to a negative value,
 * but this just gives a safe false positive
 */

#  define _EXTEND_NEEDS_GROW(p,n)     ((n) < 0 || PL_stack_max - (p) < (n))


/* EXTEND_SKIP(): used for where you would normally call EXTEND(), but you know
 * for sure that a previous op will have already extended the stack
 * sufficiently. For example pp_enteriter ensures that there is always at least
 * 1 free slot, so pp_iter can return &PL_sv_yes/no without checking each time.
 * Calling EXTEND_SKIP() defeats the HWM debugging mechanism which would
 * otherwise whine
 */
...
/* set TARG to the IV value i. If do_taint is false, assume that PL_tainted can
 * never be true */
...
/* set TARG to the UV value u. If do_taint is false, assume that PL_tainted can
 * never be true */
...
/* set TARG to the NV value n. If do_taint is false, assume that PL_tainted can
 * never be true */
