 ...
 *    that (loosely speaking) are statement separators. They hold information
 *    important for lexical state and error reporting. At run time, PL_curcop
 *    is set to point to the most recently executed cop, and thus can be used
 ...
 * longjmp() at that runlevel (we make sure PL_start_env.je_prev is always null
 * to ensure this).
 *
 ...
 *
 * top_env needs to be non-zero. It points to an area in which longjmp() stuff
 * is stored, as C callstack info there at least is thread specific this has to
 * be per-thread. Otherwise a 'die' in a thread gives that thread the C stack
 * of last thread to do an eval {}!
*/
 ...
 *
 * All the flexible exceptions code has been removed. See the following threads
 * for details:
 *
 ...
 *
 * https://github.com/Perl/perl5/commit/312caa8e97f1c7ee342a9895c2f0e749625b492
 * 9
 * https://github.com/Perl/perl5/commit/14dd3ad8c9bf82cf09798a22cc89a9862dfd6d1
 * a
*/
...
These look up the entry in the cop hints hash C<cophh> with the key specified
by C<key> (and C<keylen> in the C<pvn> form), returning that value as a mortal
...
(and C<keylen> in the C<pvn> form), returning true if a value exists, and false
otherwise.
...
key/value pairs in the cop hints hash C<cophh>. C<flags> is currently unused
and must be zero.
...
These store a value, associated with a key, in the cop hints hash C<cophh>, and
return the modified hash. The returned hash pointer is in general not the same
as the hash pointer that was passed in. The input hash is consumed by the
function, and the pointer to it must not be subsequently used. Use
L</cophh_copy> if you need both hashes.

C<value> is the scalar value to store for this key. C<value> is copied by these
functions, which thus do not take ownership of any reference to it, and hence
later changes to the scalar will not be reflected in the value visible in the
cop hints hash. Complex types of scalar will not be stored with referential
integrity, but will be coerced to strings.

The forms differ in how the key is specified. In all forms, the key is pointed
to by C<key>. In the plain C<pv> form, the key is a C language NUL-terminated
string. In the C<pvs> form, the key is a C language string literal. In the
C<pvn> form, an additional parameter, C<keylen>, specifies the length of the
string, which hence, may contain embedded-NUL characters. In the C<sv> form,
C<*key> is an SV, and the key is the PV extracted from that. using
C<L</SvPV_const>>.
...
and return the modified hash. The returned hash pointer is in general not the
same as the hash pointer that was passed in. The input hash is consumed by the
function, and the pointer to it must not be subsequently used. Use
L</cophh_copy> if you need both hashes.
...
struct cop {
    ...
    /* for now just a bitmask stored here. If we get sufficient features this
       may become a pointer. How these flags are stored is subject to change
       without notice. Use the macros to test for features.
     */
    U32         cop_features;
};
...
Available only on unthreaded perls. Makes C<pv> the name of the file associated
with the C<COP> C<c>
...
#ifdef DEBUGGING
/* on debugging builds, poison cx afterwards so we know no code uses it -
 * because after doing cxstack_ix--, any ties, exceptions etc may overwrite the
 * current stack frame */
#  define CX_POP(cx)            \
       assert(CX_CUR() == cx);  \
       cxstack_ix--;            \
...
/* When micro-optimising :-) keep GIVEN next to the LOOPs, as these 5 share a
   jump table in pp_ctl.c The first 4 don't have a 'case' in at least one
   switch statement in pp_ctl.c
 */
#define CXt_GIVEN              3
...
/* Support for switching (stack and block) contexts. This ensures magic doesn't
 * invalidate local stack and cx pointers. Which one to use (or add) is mostly,
 * but not completely arbitrary: See
...
Opening bracket for a lightweight callback. See L<perlcall/LIGHTWEIGHT
CALLBACKS>.
...
Closing bracket for a lightweight callback. See L<perlcall/LIGHTWEIGHT
CALLBACKS>.
...
/* Like PUSH_MULTICALL, but allows you to specify extra flags for the CX stack
 * entry (this isn't part of the public API) */
...
/* Change the CV of an already-pushed MULTICALL CxSUB block. (this isn't part
 * of the public API) */
