...
L<perl5-porters@perl.org|mailto:perl5-porters@perl.org>. It may be that someone
there will point out a way to accomplish what you need without using an
...
/* XXX signals are process-wide anyway, so we ignore the implications of this
 * for threading */
...
#ifndef PERL_MICRO
/* If Perl has to ignore SIGPFE, this is its saved state. See perl.h macros
 * PERL_FPU_INIT and PERL_FPU_{PRE,POST}_EXEC. */
...
PERLVARI(G, perlio_debug_fd, int, 0)    /* the fd to write perlio debug into, 0
                                           means not set yet */
#endif
...
appropriate element of this array. The new op is passed in as the sole argument
to the check function, and the check function returns the completed op. The
check function may (as the name suggests) check the op for validity and signal
errors. It may also initialise or modify parts of the ops, or perform more
radical surgery such as adding or removing child ops, or even throw the op away
...
custom behaviour. In the usual case where it decides not to do anything special
with an op, it must chain the preexisting op function. Check functions are thus
linked in a chain, with the core's base checker at the end.
...
C<PL_phase> was introduced in Perl 5.14; in prior perls you can use C<PL_dirty>
(boolean) to determine whether the interpreter is in global destruction. (Use
of C<PL_dirty> is discouraged since 5.14.)
...
/* this is currently set without MUTEX protection, so keep it a type which can
 * be set atomically (ie not a bit field) */
...
C<keyword_ptr> points at the word in the parser's input buffer, and
...
stored in C<*op_ptr>. The function then returns a constant indicating the
...
side effects. It may modify C<%^H>, define functions, and so on. Typically, if
side effects are the main purpose of a handler, it does not wish to generate
...
own function pointer to it. Your handler function should look for keywords that
it is interested in and handle those. Where it is not interested, it should
call the saved plugin function, passing on the arguments it received. Thus
C<PL_keyword_plugin> actually points at a chain of handler functions, all of
which have an opportunity to handle keywords, and only the last function in the
...
/* Restricted hashes placeholder value. In theory, the contents are never used,
   only the address. In practice, &PL_sv_placeholder is returned by some APIs,
   and the calling code is checking SvOK(). */
...
/* these record the best way to perform certain IO operations while atomically
 * setting FD_CLOEXEC. On the first call, a probe is done and the result
 * recorded for use by subsequent calls. In theory these variables aren't
 * thread-safe, but the worst that can happen is that two treads will both do
 * an initial probe
 */
