 ...
 *    2002, 2004, 2005, 2006, 2007, 2008, 2012, 2013, 2014, 2015, 2016, 2017,
...
C<(bool)> cast didn't necessarily do the right thing, so this macro was created
(and made somewhat complicated to work around bugs in old compilers). Now, many
years later, and C99 is used, this is no longer required, but is kept for
...
/* Try to figure out __func__ or __FUNCTION__ equivalent, if any. XXX Should
 * really be a Configure probe, with HAS__FUNCTION__ and FUNCTION__ as results.
...
#  define SAFE_FUNCTION__     __func__
                               ...
                               modern enough cc (in Tru64, -c99 not known, only
                               -std1). */
#  define FUNCTION__          ("")
...
/* XXX A note on the perl source internal type system. The original intent was
that I32 be *exactly* 32 bits.

Currently, we only guarantee that I32 is *at least* 32 bits. Specifically, if
int is 64 bits, then so is I32. (This is the case for the Cray.) This has the
advantage of meshing nicely with standard library calls (where we pass an I32
and the library is expecting an int), but the disadvantage that an I32 is not
32 bits. Andy Dougherty August 1996

There is no guarantee that there is *any* integral type with exactly 32 bits.
It is perfectly legal for a system to have sizeof(short) == sizeof(int) ==
sizeof(long) == 8.

Similarly, there is no guarantee that I16 and U16 have exactly 16 bits.

For dealing with issues that may arise from various 32/64-bit systems, we will
ask Configure to check out
 ...
 * (2**0, 2**1, etc). In other words if it has just a single bit set. If not,
 * subtracting 1 would leave the uppermost bit set, so the & would yield
 * non-zero */
#if defined(PERL_CORE) || defined(PERL_EXT)
...
comma (hence the comma operator). If we just used a straight assert(), we would
get a comma with nothing before it when not DEBUGGING.
...
Each has two main parameters, string-like operands that are compared against
each other, as specified by the macro name. Some macros may additionally have
one or potentially even two length parameters. If a length parameter applies to
both string parameters, it will be positioned third; otherwise any length
parameter immediately follows the string parameter it applies to.

If the prefix to the name is 'str', the string parameter is a pointer to a C
language string. Such a string does not contain embedded NUL bytes; its length
may be unknown, but can be calculated by C<strlen()>, since it is terminated by
a NUL, which isn't included in its length.

The optional 'n' following 'str' means that there is a third parameter, giving
the maximum number of bytes to look at in each string. Even if both strings are
longer than the length parameter, those extra bytes will be unexamined.

The 's' suffix means that the 2nd byte string parameter is a literal C
double-quoted string. Its length will automatically be calculated by the macro,
so no length parameter will ever be needed for it.

If the prefix is 'mem', the string parameters don't have to be C strings; they
may contain embedded NUL bytes, do not necessarily have a terminating NUL, and
their lengths can be known only through other means, which in practice are
additional parameter(s) passed to the function. All 'mem' functions have at
least one length parameter. Barring any 'l' or 's' suffix, there is a single
length parameter, in position 3, which applies to both string parameters. The
's' suffix means, as described above, that the 2nd string is a literal
double-quoted C string (hence its length is calculated by the macro, and the
length parameter to the function applies just to the first string parameter,
and hence is positioned just after it). An 'l' suffix means that the 2nd string
parameter has its own length parameter, and the signature will look like
memFOOl(s1, l1, s2, l2).

BEGIN (and END) are for testing if the 2nd string is an initial (or final)
substring of the 1st string. 'P' if present indicates that the substring must
be a "proper" one in tha mathematical sense that the first one must be strictly
larger than the 2nd.
*/
 ...
 * isupper(), etc. to tell the truth. And when it comes to /\w+/ with tainting
 * enabled, we *must* be able to trust our character classes.
 *
...
to regular expression character classes. (See L<perlrecharclass/POSIX Character
Classes>.) There are several variants for each class. (Not all macros have all
variants; each item below lists the ones valid for it.) None are affected by
C<use bytes>, and only the ones with C<LC> in the name are affected by the
current locale.
...
with no suffix C<"_A">. This variant is used to emphasize by its name that only
ASCII-range characters can return TRUE.
...
the same thing. Each call to one of these classifies the first character of the
string starting at C<p>. The second parameter, C<e>, points to anywhere in the
string beyond the first character, up to one byte past the end of the entire
string. Although both variants are identical, the suffix C<_safe> in one name
emphasizes that it will not attempt to read beyond S<C<e - 1>>, provided that
...
stands for. If Perl can determine that the current locale is a UTF-8 locale, it
uses the published Unicode rules; otherwise, it uses the C library function
that gives the named classification. For example, C<isDIGIT_LC()> when not in a
...
C<isI<FOO>_LC_uvchr>, but are used for UTF-8 encoded strings. The two forms are
different names for the same thing. Each call to one of these classifies the
first character of the string starting at C<p>. The second parameter, C<e>,
points to anywhere in the string beyond the first character, up to one byte
past the end of the entire string. Although both variants are identical, the
suffix C<_safe> in one name emphasizes that it will not attempt to read beyond
S<C<e - 1>>, provided that the constraint S<C<s E<lt> e>> is true (this is
asserted for in C<-DDEBUGGING> builds). If the UTF-8 for the input character is
malformed in some way, the program may croak, or the function may return FALSE,
at the discretion of the implementation, and subject to change in future
releases.
...
analogous to C<m/[[:alpha:]]/>. See the L<top of this section|/Character
classification> for an explanation of the variants.
...
C<[A-Za-z0-9]>, analogous to C<m/[[:alnum:]]/>. See the L<top of this
section|/Character classification> for an explanation of the variants.
...
explanation of the variants. Note, however, that some platforms do not have the
C library routine C<isascii()>. In these cases, the variants whose names
...
considered to be a blank, analogous to C<m/[[:blank:]]/>. See the L<top of this
section|/Character classification> for an explanation of the variants. Note,
however, that some platforms do not have the C library routine C<isblank()>. In
these cases, the variants whose names contain C<LC> are the same as the
...
character, analogous to C<m/[[:cntrl:]]/>. See the L<top of this
section|/Character classification> for an explanation of the variants. On
EBCDIC platforms, you almost always want to use the C<isCNTRL_L1> variant.
...
identical to C<isDIGIT>. See the L<top of this section|/Character
classification> for an explanation of the variants.
...
character, analogous to C<m/[[:graph:]]/>. See the L<top of this
section|/Character classification> for an explanation of the variants.
...
character, analogous to C<m/[[:lower:]]/>. See the L<top of this
section|/Character classification> for an explanation of the variants
...
L<perlrecharclass/POSIX Character Classes> for details. See the L<top of this
section|/Character classification> for an explanation of the variants.
...
character. This is analogous to what C<m/\s/> matches in a regular expression.
Starting in Perl 5.18 this also matches what C<m/[[:space:]]/> does. Prior to
5.18, only the locale forms of this macro (the ones with C<LC> in their names)
matched precisely what C<m/[[:space:]]/> does. In those releases, the only
difference, in the non-locale variants, was that C<isSPACE()> did not match a
vertical tab. (See L</isPSXSPC> for a macro that matches a vertical tab in all
releases.) See the L<top of this section|/Character classification> for an
...
the corresponding C<isSPACE()> macros. The locale forms of this macro are
...
character, analogous to C<m/[[:upper:]]/>. See the L<top of this
section|/Character classification> for an explanation of the variants.
...
character, analogous to C<m/[[:print:]]/>. See the L<top of this
section|/Character classification> for an explanation of the variants.
...
in a regular expression. A word character is an alphabetic character, a decimal
digit, a connecting punctuation character (such as an underscore), or a "mark"
...
and C<isXDIGIT_L1()> are identical to C<isXDIGIT()>. See the L<top of this
section|/Character classification> for an explanation of the variants.
...
the official Unicode property C<XID_Start>. The difference is that this returns
true only if the input character also matches L</isWORDCHAR>. See the L<top of
this section|/Character classification> for an explanation of the variants.
...
that this returns true only if the input character also matches L</isWORDCHAR>.
See the L<top of this section|/Character classification> for an explanation of
the variants.
...
character sequence C<SS>. This presents some complications The lowercase of all
characters in the range 0..255 is a single character, and thus
C<L</toLOWER_L1>> is furnished. But, C<toUPPER_L1> can't exist, as it couldn't
return a valid result for all legal inputs. Instead C<L</toUPPER_uvchr>> has an
API that does allow every possible legal result to be returned.) Likewise no
other function that is crippled by not being able to give the correct results
for the full range of possible inputs has been implemented here.
...
range. The uppercase of the vast majority of Unicode code points is the same as
...
NOTE: the uppercase of a code point may be more than one code point. The return
value of this function is only the first of these. The entire uppercase is
...
lowercase. All other inputs are returned unchanged. Since these are macros, the
input type may be any integral one, and the output will occupy the same number
of bits as the input.
...
range. The lowercase of the vast majority of Unicode code points is the same as
the code point itself. For these, and for code points above the legal Unicode
maximum, this returns the input code point unchanged. It additionally stores
the UTF-8 of the result into the buffer beginning at C<s>, and its length in
bytes into C<*lenp>. The caller must have made C<s> large enough to contain at
...
NOTE: the lowercase of a code point may be more than one code point. The return
value of this function is only the first of these. The entire lowercase is
returned in C<s>. To determine if the result is more than a single code point,
you can do something like this:
...
titlecase of any lowercase ASCII-range code point. In this range, the titlecase
is identical to the uppercase. All other inputs are returned unchanged. Since
...
range. The titlecase of the vast majority of Unicode code points is the same as
the code point itself. For these, and for code points above the legal Unicode
maximum, this returns the input code point unchanged. It additionally stores
the UTF-8 of the result into the buffer beginning at C<s>, and its length in
bytes into C<*lenp>. The caller must have made C<s> large enough to contain at
...
NOTE: the titlecase of a code point may be more than one code point. The return
value of this function is only the first of these. The entire titlecase is
returned in C<s>. To determine if the result is more than a single code point,
you can do something like this:
 ...
 * lower 8. It is designed to be hopefully bomb-proof, making sure that no bits
 * of information are lost even on a 64-bit machine, but to get the compiler to
 ...
 * guarantees that this evaluates to a constant true at compile time.
 *
...
/* Returns true if c is in the range l..u, where 'l' is non-negative Written
 * this way so that after optimization, only one conditional test is needed. */
...
#   endif
     ...
     * unsigned type. khw supposes that it could be written as && ((c) == '\0'
     * || (c) > 0) to avoid the message, but the cast will likely avoid extra
     * branches even with stupid compilers. */
...
/* Character class numbers. For internal core Perl use only. The ones less than
 * 32 are used in PL_charclass[] and the ones up through the one that
 ...
 * their actual definitions are here. If that file has a name not used here, it
 * won't compile.
 *
 ...
 * (in reginclass() in regexec.c). Except \v should be last, as it isn't a real
 * Posix character class, and some (small) inefficiencies in regular expression
 * handling would be introduced by putting it in the middle of those that are.
 * Also, cntrl and ascii come after the others as it may be useful to group
 * these which have no members that match above Latin1, (or above ASCII in the
 * latter case) */
 ...
 * non-64bit QUAD_IS_INT systems, using some #ifdefs to distinguish between
 * having a 2nd word or not. The IS_IN_SOME_FOLD bit is the most easily
 * expendable, as it is used only for optimization (as of this writing), and
 * differs in the Latin1 range from the ALPHA bit only in two relatively
 * unimportant characters: the masculine and feminine ordinal indicators, so
 * removing it would just cause /i regexes which match them to run less
 * efficiently. Similarly the EBCDIC-only bits are used just for speed, and
 * could be replaced by other means */

#if defined(PERL_CORE) || defined(PERL_EXT)
/* An enum version of the character class numbers, to help compilers optimize
 */
typedef enum {
...
/* ASCII casing. These could also be written as #define toLOWER(c) (isASCII(c)
? toLOWER_LATIN1(c) : (c)) #define toUPPER(c) (isASCII(c) ?
...
/* Modified uc. Is correct uc except for three non-ascii chars which are all
 * mapped to one of them, and these need special handling; returns the input
 ...
 * ispunct(), and things that are \W, like ispunct(), arent't controls. Not all
 * possible weirdnesses are checked for, just ones that were detected on actual
...
/* Below is the next level up, which currently expands to nothing more than the
 * previous layer. These are the macros to use if you really need something
 * whose input domain is a byte, and the locale isn't UTF-8; that is, where you
 * normally would have to use things like bare isalnum().
 *
 * But most likely you should instead use the layer defined further below which
 * has names like isALPHA_LC. They deal with larger-than-byte inputs, and UTF-8
 * locales.
 *
 ...
 * locale. It returns a single value, so can't adequately return the upper case
 ...
 * list binary search. generic_invlist_uvchr_() can be used for classes where
 * that overhead is faster than a direct lookup. generic_uvchr_() won't compile
 * if 'c' isn't unsigned, as it won't match the 'above_latin1' prototype.
 * generic_isCC_() macro does bounds checking, so have duplicate checks here,
 * so could create versions of the macros that don't, but experiments show that
 * gcc optimizes them out anyway. */
 ...
 * DEBUGGING builds. Much code that calls these depends on this being true, for
 * other reasons. toke.c is treated specially as using the regular assertion
 * breaks it in many ways. All strings that these operate on there are supposed
 * to have an extra NUL character at the end, so that *e = \0. A bunch of code
 * in toke.c assumes that this is true, so the assertion allows for that */
 ...
 * IDFIRST is an alnum. See https://github.com/Perl/perl5/issues/10275 for more
 * detail than you ever wanted to know about. (In the ASCII range, there isn't
 * a difference.) This used to be not the XID version, but we decided to go
 * with the more modern Unicode definition */
 ...
 * 4 on both systems, but that is EOT on ASCII; ST on EBCDIC. '?' is
 * special-cased on EBCDIC to APC, which is the control there that is the
 * outlier from the block that contains the other controls, just like
 * toCTRL('?') on ASCII yields DEL, the control that is the outlier from the C0
 * block. If it weren't special cased, it would yield a non-control. The
 * conversion works both ways, so toCTRL('D') is 4, and toCTRL(4) is D, etc. */
#ifndef EBCDIC
 ...
 * a-f) to its numeric value without using any branches. The input is validated
 * only by an assert() in DEBUGGING builds.
 *
 ...
 * position, and then to the eights position. Both are added together to form 0
 * if the input is '0'-'9' and to form 9 if alpha. This is added to the final
 * four bits of the input to form the correct value. */
...
parameter, I<x>, a debug aid which allowed callers to identify themselves. This
aid has been superseded by a new build option, PERL_MEM_LOG (see
...
/* This expression will be constant-folded at compile time. It checks whether
 * or not the type of the count n is so small (e.g. U8 or U16, or U32 on 64-bit
 * systems) that there's no way a wrap-around could occur. As well as avoiding
 * the need for a run-time check in some cases, it's designed to avoid compiler
 * warnings like: comparison is always false due to limited range of data type
 * It's mathematically equivalent to max(n) * sizeof(t) > MEM_SIZE_MAX
 */
...
/* This is written in a slightly odd way to avoid various spurious compiler
 * warnings. We *want* to write the expression as
 * _MEM_WRAP_NEEDS_RUNTIME_CHECK(n,t) && (n > C) (for some compile-time
 * constant C), but even when the LHS constant-folds to false at compile-time,
 * g++ insists on emitting warnings about the RHS (e.g. "comparison is always
 * false"), so instead we write it as
 *
 *    (cond ? n : X) > C
 *
 * where X is a constant with X > C always false. Choosing a value for X is
 * tricky. If 0, some compilers will complain about 0 > C always being false;
 * if 1, Coverity complains when n happens to be the constant value '1', that
 * cond ? 1 : 1 has the same value on both branches; so use C for X and hope
...
/* Perl_deprecate was not part of the public API, and did not have a
   deprecate() shortcut macro defined without -DPERL_CORE. Neither
   codesearch.google.com nor CPAN::Unpack show any users outside the core. */
#ifdef PERL_CORE
#  define deprecate(s)                                      \
 ...
 * suite by L’Ecuyer and Simard. (Note that Drand48 also fails BigCrush). The
 * main point is they produce different sequences and in places where we want
 * some randomlike behavior they are cheap and easy.
 *
...
/* 64 bit version - currently this is unused, it is provided here to complement
 * the 32 bit _B variant which IS used. */
