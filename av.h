 ...
 * initialized such that any element can be retrieved as a SV*. Such AVs may be
 * referred to as "real" AVs. Examples include regular perl arrays, tiedarrays
 * (since v5.16), and padlist AVs.
 *
 ...
 * elements are safe for arbitrary access. This type of AV may be referred to
 * as "fake" AVs. Examples include "@_" (unless tied), the scratchpad list, and
 * the backrefs list on an object or stash.
 *
 * SVpav_REIFY is only meaningful on such "fake" AVs (i.e. where SVpav_REAL is
 * not set). It indicates that the fake AV is capable of becoming real if the
 * array needs to be modified in some way. Functions that modify fake AVs check
 * both flags to call av_reify() as appropriate.
 *
 ...
 * incremented.
 *
 * Note that the Perl stack has neither flag set. (Thus, items that go on the
 * stack are never refcounted.)
 *
...
This is useful for doing pointer arithmetic on the array. If all you need is to
look up an array element, then prefer C<av_fetch>.
...
/* Note that it doesn't make sense to do this: SvGETMAGIC(av); IV x =
 * av_tindex_nomg(av);
 */
...
A C language array of pointers to the individual elements. These are treated as
